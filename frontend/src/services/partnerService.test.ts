...
describe('partnerService', () => {
  ...
  describe('verifyInvitation', () => {
    it('should verify invitation token', async () => {
      ...
      const result = await partnerService.verifyInvitation('valid-token');

      expect(api.get).toHaveBeenCalledWith('/partners/invitation/verify?token=valid-token', true);
      expect(result).toEqual(mockResponse);
    });

    it('should handle expired invitation', async () => {
      vi.mocked(api.get).mockRejectedValueOnce(new Error('Invitation expired'));

      await expect(partnerService.verifyInvitation('expired-token')).rejects.toThrow('Invitation expired');
    });

    it('should handle invalid invitation token', async () => {
      vi.mocked(api.get).mockRejectedValueOnce(new Error('Invalid invitation token'));

      await expect(partnerService.verifyInvitation('invalid-token')).rejects.toThrow('Invalid invitation token');
    });
  });
  ...
  describe('acceptInvitation', () => {
    it('should accept invitation', async () => {
      ...
      const result = await partnerService.acceptInvitation('valid-token', 'user-1');

      expect(api.post).toHaveBeenCalledWith(
        '/partners/invitation/accept',
        { token: 'valid-token', userId: 'user-1' },
        true
      );
      expect(result).toEqual(mockPartner);
    });
    ...
    it('should handle already accepted invitation', async () => {
      vi.mocked(api.post).mockRejectedValueOnce(new Error('Invitation already accepted'));

      await expect(
        partnerService.acceptInvitation('used-token', 'user-1')
      ).rejects.toThrow('Invitation already accepted');
    });
  });

  describe('registerWithInvitation', () => {
    it('should register new user with invitation', async () => {
      const mockResponse = {
        message: 'Registration successful',
        user: { id: 'user-1', email: 'new@example.com', firstName: 'New', lastName: 'User' },
        partner: { id: 'partner-1', name: 'Partner', email: 'new@example.com' },
        session: {
          accessToken: 'access-token',
          refreshToken: 'refresh-token',
          expiresIn: 3600,
          expiresAt: 1704067199,
        },
      };
    ...
    it('should handle invalid token error', async () => {
      vi.mocked(api.post).mockRejectedValueOnce(new Error('Invalid or expired token'));

      await expect(
        partnerService.registerWithInvitation({
          token: 'invalid-token',
          password: 'password123',
          firstName: 'New',
          lastName: 'User',
    ...
    it('should handle weak password error', async () => {
      vi.mocked(api.post).mockRejectedValueOnce(new Error('Password too weak'));
  ...
  describe('Edge cases', () => {
    ...
    it('should handle invitation token with special characters', async () => {
      vi.mocked(api.get).mockResolvedValueOnce({
        success: true,
        data: {
          invitation: { id: '1', email: 'test@example.com', expiresAt: '2024-12-31' },
          partner: { id: '1', name: 'Partner' },
        },
      });

      await partnerService.verifyInvitation('token-with-special-chars-123');

      expect(api.get).toHaveBeenCalledWith(
        '/partners/invitation/verify?token=token-with-special-chars-123',
        true
      );
    });
  });
